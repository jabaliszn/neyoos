...
async function main() {
  ...
  console.log("Loading academics page for Timetable Generator screenshot...");
  await page.goto(`${BASE}/academics`, { waitUntil: "domcontentloaded" }).catch(() => {});
  ...
  // Click on the Timetable Generator tab
  console.log("Clicking Timetable Generator tab...");
  await page.getByRole("button", { name: "Timetable Generator", exact: true }).click();
  await page.waitForTimeout(2000);

  const desktopPath = path.join(OUT, "timetable-generator-g18.png");
  await page.screenshot({ path: desktopPath, fullPage: false });
  console.log("Saved academics page screenshot with G.18 Timetable Generator to:", desktopPath);
