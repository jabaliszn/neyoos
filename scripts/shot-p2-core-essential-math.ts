...
async function main() {
  ...
  const ctx = await browser.newContext({ viewport: { width: 1920, height: 1300 }, deviceScaleFactor: 1 });
  ...
  // Scroll to a STEM pathway card to show "Core Mathematics" CORE badge clearly.
  await page.evaluate(() => window.scrollBy(0, 500));
  await page.waitForTimeout(500);
  await page.screenshot({ path: path.join(OUT, "p2-core-essential-math.png"), fullPage: false });
  await browser.close();
  console.log("✅ Screenshot captured: screenshots/p2-core-essential-math.png");
}
