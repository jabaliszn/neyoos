/**
 * G.10 Student ID Card PDF download (QR-verified, co-branded).
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Row-scoping: a PARENT can download their own child's ID card, not others'.
    if (!(await canViewStudent(user, params.id)))
      throw new StudentError("NOT_FOUND", "Student not found.");
      
    const { pdf, fileName } = await buildStudentIdCardPdf(user.tenantId, params.id);
