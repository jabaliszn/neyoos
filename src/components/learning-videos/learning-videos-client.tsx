...
const IDEAS = ["algebra basics", "photosynthesis", "KCSE English set books", "fractions Grade 6", "CBC creative arts", "electricity Form 2"];
...
export function LearningVideosClient() {
  ...
  function useIdea(idea: string) { setQ(idea); void load(idea); }
    ...
    <Card>
      <CardContent className="space-y-3 p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {IDEAS.map((idea) => <button key={idea} onClick={() => useIdea(idea)} className="rounded-full border border-navy-100 bg-white/70 px-3 py-1.5 text-xs font-semibold text-navy-600 hover:bg-green-50 hover:text-green-800 dark:border-navy-800 dark:bg-navy-950/40 dark:text-navy-300">{idea}</button>)}
        </div>
