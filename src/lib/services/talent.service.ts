...
/**
 * P.4 REPAIR (2026-07-02, found while live-verifying the unrelated P.4
 * feature): every exported function in this file called tenantDb() without
 * ever establishing a withTenant() scope — the exact same PRE-EXISTING bug
 * class found and fixed in pathway.service.ts during P.1 ("No tenant in
 * scope" thrown at runtime). Confirmed via `git diff`/`git log` this predates
 * the current session; discovered because it crashed the real Student
 * Profile page's Talent tab (a genuine live error, not a hypothetical one)
 * while verifying an unrelated P.4 feature on the same page. Fixed using
 * the identical repair pattern as P.1: wrap every exported function's body
 * in withTenant(user.tenantId, ...) inside the service file itself, so it
