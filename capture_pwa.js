...
(async () => {
  ...
  try {
    ...
    await page.waitForTimeout(3000); // Wait for page elements and PWA installer pill to render

    console.log("Triggering expanded vertically-taller Siri-Style Dynamic Island...");
    await page.evaluate(() => {
      if (typeof window !== 'undefined' && (window).__toast) {
        (window).__toast({
          title: "Bundi has compiled Saturday timetable rosters!",
          description: "All streams synced with conflict-free alternate Week A/B slots successfully.",
          tone: "success"
        });
      }
    });

    await page.waitForTimeout(1000); // Wait for toast to render and animate

    console.log("Taking screenshot of the PWA Install Button and the Dynamic Island...");
    await page.screenshot({ path: '../pwa_island_screenshot.png', fullPage: false });
    console.log("Screenshot successfully saved as /home/user/pwa_island_screenshot.png!");
