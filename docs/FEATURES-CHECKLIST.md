...
- [ ] Add admin-facing explanation in NEYO Ops / School settings: “configure curriculum, do not hardcode curriculum.”
- [ ] Ensure all new education features are tenant-scoped, audit-logged and configurable.

## J.2 — Curriculum Engine (core foundation)

- [ ] Create configurable `Curriculum` model per tenant: name, country/context, active version, effective dates.
- [ ] Create configurable `EducationLevel` model: preschool, primary, junior, senior, custom levels.
- [ ] Create configurable `GradeBand` / grade names: PP1, Grade 1, Form 1, Year 7, custom names — no hardcoding.
- [ ] Create `LearningArea` model connected to subjects/strands/competencies.
- [ ] Add curriculum admin UI in Settings → Academics / Curriculum.
- [ ] Map existing `Subject`, `SchoolClass`, `AcademicTerm` to the new curriculum engine instead of replacing them.
- [ ] Migration assistant: convert existing CBC/8-4-4 seed data into configurable curriculum records.

## J.3 — Flexible Assessment Engine

- [ ] Create configurable `AssessmentType`: exam, CAT, project, practical, oral, observation, portfolio, peer assessment, self assessment, continuous assessment.
- [ ] Create `AssessmentPlan`: learning area, class/grade, term, weight, due date, rubric, competency links.
- [ ] Extend existing `Exam` / `ExamResult` or add compatible assessment layer without breaking existing report-card logic.
- [ ] Support marks, rubric levels, narrative observations and evidence files in one assessment model.
- [ ] Allow school-defined weighting rules per assessment type.
- [ ] Add moderation/release workflow for non-exam assessments.
- [ ] Parent/student visibility respects release status.

## J.4 — Competency Framework

- [ ] Create configurable `Competency` model: communication, critical thinking, problem solving, creativity, citizenship, digital literacy, learning to learn, etc.
- [ ] Create competency groups/strands per curriculum.
- [ ] Link assessments, teacher observations, projects and co-curricular activities to competencies.
- [ ] Store competency evidence over time, not just latest score.
- [ ] Add competency summary to student profile and parent dashboard.
- [ ] Add competency heatmap analytics for class/grade/school.
...
- [ ] Before building any J feature, audit existing B/I features first: Exams, CBC, Timetable, LMS, Parent Portal, Student Profile, Storage Vault, Co-curricular, Document Design.
- [ ] If existing feature is partial, extend it rather than creating a duplicate module.
- [ ] Every J feature must update this checklist and `docs/CONTEXT-ANCHOR.md`.
